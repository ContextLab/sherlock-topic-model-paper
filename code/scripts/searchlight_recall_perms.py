...
model = np.load('/idata/cdl/data/fMRI/andy/sherlock/data/s%s_recall_model_dtw.npy' % str(subid))

# shift the recall model
...
np.save('/idata/cdl/data/fMRI/andy/sherlock/analyses/searchlight_recall/perms/s%s_perm%s_shift%s' % (str(subid), str(perm), str(shift)), result)
