...
def load_cluster_output(hostname, username, password):
    datadir_local = '../../data/processed/'
    emb_path_remote = f'/dartfs/rc/lab/D/DBIC/CDL/{username}/sherlock-embedding/data/optimized/embeddings.npy'
    emb_path_local = opj(datadir_local, 'embeddings.npy')

    sl_dir_remote = f'/dartfs/rc/lab/D/DBIC/CDL/{username}/searchlights/data/'
    sl_output_dr = opj(sl_dir_remote, 'finished')
    ...
    with spurplus.connect_with_retries(hostname=hostname,
                                   ...
                                   for analysis in ['video', 'recall']:
                                       src_dir = opj(sl_output_dr, f'searchlight_{analysis}')
                                    ...
                                    # copy over reference image
                                    src_path = opj(sl_dir_remote, 'inputs', 'fMRI', 'ref_img.nii.gz')
                                    dest_path = opj(sl_vdir_local, 'ref_img.nii.gz')
                                   remote.get(src_path, dest_path)
